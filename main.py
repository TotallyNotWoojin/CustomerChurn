...
xgboost_SMOTE_model = load_model("xgboost_SMOTE2.pkl")
xgboost_featureEngineered_model = load_model('xgboost_model.pkl')
...
def make_predictions(input_df, input_dict):
  probabilities = {
      #'native_bayes':native_bayes_model.predict_proba(input_df)[0][1],
      'Random Forest': random_forest_model.predict_proba(input_df)[0][1],
      #'decision_tree':decision_tree_model.predict_proba(input_df)[0][1],
      'K-Nearest Neighbor': knn_model.predict_proba(input_df)[0][1],
      #'svm':svm_model.predict_proba(input_df)[0][1],
      #'voting_classifier':voting_classifier_model.predict_proba(input_df)[0][1],
      'XGBoost': xgboost_model.predict_proba(input_df)[0][1],
      #'XGBoost SMOTE': xgboost_SMOTE_model.predict_proba(input_df)[0][1],
      #'xgboost_featureEngineered':
...
- If the customer has over a 40% risk of churning, generate a 3 sentence explanation of_ why they are at risk of churning. 
...
- Your explanation should be based on the customer's information, the summary statistics. of churned and non-churned customers, and the feature importances provided. 
  ...
  Don't mention the probability of churning, or the machine learning model, or say anything like "Based on the machine learning model's prediction and top 10 most important features", just explain the prediction. Never ever give me multiple possibilities and just look a the average probability to make your responses. Do not analyze the different possibilities of churning, just stick to one narrative
  """
...
if selected_customer_option:
  ...
  with col1:
    ...
    gender = st.radio("Gender", ["Male", "Female"],
                      index=0 if selected_customer['Gender'] == 0 else 1)
